...
export async function startCustomerOrderSession({ guestAutoAdd, guestCount, storeId, tableId }) {
  return withOrderCommandFailureLog({
    ...
    if (shouldUseOrderCommandFunctions()) {
      return callOrderCommandFunction('startCustomerOrderSessionCommand', { guestAutoAdd, guestCount, storeId, tableId })
    }
...
export async function submitCustomerOrderItems({ items, orderId, storeId, tableId, clientRequestId }) {
  ...
  return withOrderCommandFailureLog({
    ...
    if (shouldUseOrderCommandFunctions()) {
      return callOrderCommandFunction('submitCustomerOrderItemsCommand', { items, orderId, storeId, tableId, clientRequestId: requestId })
    }
    ...
    const itemRefs = normalizedItems.map((_, index) => itemRefFor({ orderId, clientRequestId: requestId, index }))
    const now = serverTimestamp()
...
export async function submitStaffOrderItems({ cart, orderId, storeId, tableId, clientRequestId }) {
  ...
  return withOrderCommandFailureLog({
    ...
    if (shouldUseOrderCommandFunctions()) {
      return callOrderCommandFunction('submitStaffOrderItemsCommand', { cart, orderId, storeId, tableId, clientRequestId: requestId })
    }
...
export async function seatStaffOrderSession({ table, tableId, seatCount, activeStaff }) {
  return withOrderCommandFailureLog({
    ...
    if (shouldUseOrderCommandFunctions()) {
      return callOrderCommandFunction('seatStaffOrderSessionCommand', { tableId, seatCount, activeStaff })
    }
...
export async function completeCheckoutCommand({
  ...
  return withOrderCommandFailureLog({
    ...
    if (shouldUseOrderCommandFunctions()) {
      return callOrderCommandFunction('completeCheckoutCommand', {
        storeId,
        tableId,
        orderId,
        guestCount,
