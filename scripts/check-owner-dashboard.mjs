...
const ownerPage = readFileSync(new URL('../src/pages/owner/OwnerPage.jsx', import.meta.url), 'utf8')
...
assert.match(ownerStoreDashboard, /onManageStore\(store\)/)
assert.match(ownerStoreDashboard, /詳細・管理/)
assert.match(ownerStoreDashboard, /owner-store-manage-button/)
assert.match(ownerPage, /activateOwnerStore\(store\.id\)/)
assert.match(ownerPage, /navigate\('\/admin\/settings'\)/)
assert.match(adminLayout, /ownerActiveStoreId/)
assert.match(adminLayout, /代理管理:/)
assert.match(adminLayout, /オーナー一覧/)
