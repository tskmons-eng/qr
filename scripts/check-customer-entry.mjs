...
assert.match(orderEntrySource, /if \(loading \|\| error \|\| !table\)/)
assert.match(orderEntrySource, /<OrderEntryStatus loading=\{loading\} error=\{error\} \/>/)
assert.match(orderEntrySource, /storeConfigLoading: configLoading/)
assert.match(orderEntrySource, /const MenuPage = lazy\(\(\) => import\('\.\/MenuPage'\)\)/)
assert.match(orderEntrySource, /<Suspense fallback=\{<AppRouteLoading compact \/>\}>/)
...
assert.match(guestCountPageSource, /autoAddNote=\{showAutoAddButton \? `\$\{guestAutoAdd\.productNameSnapshot \|\| '設定メニュー'\}を\$\{count\}名分追加します` : ''\}/)
assert.match(guestCountPageSource, /ready=\{!storeConfigLoading\}/)
assert.match(guestCountPageSource, /if \(storeConfigLoading \|\| loading\) return/)
...
assert.match(guestCountSelectorSource, /ready \? `\$\{count\}名で注文を始める`/)
assert.match(guestCountSelectorSource, /disabled=\{loading \|\| !ready\}/)
