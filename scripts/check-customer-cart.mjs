...
assert.match(bottomNavSource, /checkoutLabel \?\? '注文確認'/)
assert.match(bottomNavSource, /const checkoutNavIcon = checkoutDisabled \|\| onCheckout \? '会計' : '確認'/)
...
assert.match(customerCategoryTabsSource, /CATEGORY_GROUP_LABELS/)
assert.match(customerCategoryTabsSource, /drink: 'ドリンク'/)
assert.match(customerCategoryTabsSource, /food: 'フード'/)
assert.match(customerCategoryTabsSource, /customer-category-tabs__group--\$\{category\.group\}/)
assert.match(customerMenuStyles, /\.customer-category-tabs__group/)
assert.match(customerMenuStyles, /\.customer-category-tabs__group--drink/)
assert.match(customerMenuStyles, /\.customer-category-tabs__group--food/)
...
assert.match(customerMenuProductListSource, /onKeyDown: handleRowKeyDown/)
assert.match(customerMenuProductListSource, /className=\{`customer-product\$\{rowTapEnabled \? ' is-tappable' : ''\}`\}/)
assert.ok(!customerMenuProductListSource.includes("const TapArea = rowTapEnabled ? 'button' : 'div'"), 'row tap should be on the product row, not only a nested tap area')
